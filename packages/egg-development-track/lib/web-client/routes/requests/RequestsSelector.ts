...
const getMessagesLookup: (state: IStoreState) => IMessagesLookup = (() => {
    ...
    return (state: IStoreState): IMessagesLookup => {
        ...
        console.time('[PERF] RequestsSelector.getMessagesLookup'); // tslint:disable-line:no-console
        ...
        const newListing = state.session.messages.listing.filter(message => {
            const isCleared = clearedRequestsMap[message.context.id];
        ...
        if (newListing.length > 0) {
            ...
            newListing.forEach((message: IMessage<{}>) => {
                const context = newByContextId[message.context.id] || { id: message.context.id, listing: [], byType: {} };
        ...
        console.timeEnd('[PERF] RequestsSelector.getMessagesLookup'); // tslint:disable-line:no-console
...
export const getRequestsLookup: (state: IStoreState) => IRequestsLookup = (() => {
    ...
    return (state: IStoreState): IRequestsLookup => {
        ...
        console.time('[PERF] RequestsSelector.getRequestsLookup'); // tslint:disable-line:no-console
        ...
        newContextIds.forEach((contextId: string) => {
            ...
            const webRequest = getSingleMessageByType<IWebRequestPayload>(context.byType, 'web-request');
            const webResponse = getSingleMessageByType<IWebResponsePayload>(context.byType, 'web-response');
            if (webRequest && webResponse) {
                ...
                sortNeeded = sortNeeded || (newListing.length > 0
                    && requestContext.requestStartStamp < newListing[newListing.length - 1].requestStartStamp);
        ...
        console.timeEnd('[PERF] RequestsSelector.getRequestsLookup'); // tslint:disable-line:no-console
...
export const getSelectedContextId: (state: IStoreState) => string = state => state.session.messages.selectedContextId;
