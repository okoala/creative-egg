...
const getFilters: (state: IStoreState) => IRequestsFilterState = state => state.persisted.global.requests.filter;
...
export const getFilteredRequests: (state: IStoreState) => IRequestFilterDetails = (function () {
    ...
    function incrementContentTypeClassCount(target: { [key: string]: number}, value: number) {
        ...
        if ((value & (value - 1)) === 0) { // tslint:disable-line:no-bitwise
            incrementCount(target, value);
        }
        else {
            for (let i = 0; i < 32; i++) {
                const mask = 1 << i; // tslint:disable-line:no-bitwise
                if ((mask & value) !== 0) { // tslint:disable-line:no-bitwise
    ...
    return createSelector(
        ...
        (requestLookups: IRequestsLookup, filters: IRequestsFilterState) => {
            console.time('[PERF] RequestsFilterdSelectors.getFilteredRequests'); // tslint:disable-line:no-console
            ...
            requestLookups.listing.forEach(request => {
                ...
                if (request.webResponse.url.indexOf('/sockjs-node/info') === -1
                    && request.webRequest.url.indexOf('/glimpse/') === -1
                    && !(request.webResponse.url.substr(-'.map'.length) === '.map' && isDataType(request.mediaType.contentTypeClass))) {
                        // since the above filters are in play this counter need to track
                        // the total number of requests
                        totalCount++;

                        const method = request.webRequest.method;
                        ...
                        // user driven filters
                        if (!filters.method[method]
                            && !filters.status[status]
                            && !((contentTypeClass & filters.contentTypeClass) === 0)) { // tslint:disable-line:no-bitwise
                                filteredCount++;
            ...
            console.timeEnd('[PERF] RequestsFilterdSelectors.getFilteredRequests'); // tslint:disable-line:no-console
