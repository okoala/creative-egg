...
export default class RequestResourceTable extends React.Component<IRequestResourceTableProps, {}> {
    public render() {
        const { labelHeader, values, valueHeader, fixedPoints } = this.props;
        ...
        const totalValue = roundWithFixedPoints(valueSortedTypes.reduce((prev, curr) => prev + curr.value, 0), fixedPoints);
        ...
        return (
            <div>
                <table className={commonStyles.table}>
                    ...
                    {
                        hasData
                            ? this.renderResourceRows(valueSortedTypes, totalValue, alphaSortedTypes)
                    ...
                    <tbody>
                        <tr>
                            ...
                            <td className={classNames(styles.requestResourceTableTotalCell, styles.requestResourceTableValueColumn)}>
    ...
    private renderResourceType(resourceType: IRequestResourceType, totalValue: number, sortedResourceTypes: IRequestResourceType[]) {
        ...
        return (
            <div className={styles.requestResourceTableType}>
                <div className={classNames(styles.requestResourceTableTypeIndicator, this.getResourceTypeStyle(resourceType, sortedResourceTypes))}></div>
                <span
    ...
    private renderResourceRows(resourceTypes: IRequestResourceType[], totalValue: number, sortedResourceTypes: IRequestResourceType[]) {
        return resourceTypes.map(resourceType => {
            return (
                <tr key={resourceType.type}>
                    <td>{this.renderResourceType(resourceType, totalValue, sortedResourceTypes)}</td>
                    <td className={styles.requestResourceTableValueColumn}>{toStringWithFixedPoints(resourceType.value, this.props.fixedPoints)}</td>
                </tr>
            );
        });
    }
    ...
    private renderNoDataRow() {
        return (
            <tr>
                <td colSpan={2}><InformationLabel text="No data found" annotation="Glimpse did not capture data for this request." /></td>
            </tr>
        );
    }

    private getResourceTypeStyle(resourceType: IRequestResourceType, sortedResourceTypes: IRequestResourceType[]): string {
