        ...
        return (
            <div className={styles.view}>
                ...
                <div className={styles.codeBlock}>
                    <CodeView language="json" code={this.prettyPrintJson(metadata)} elementId={['debug', 'metadata']} requestId={selectedContext.id} />
                </div>
                <h3 className={commonStyles.detailTitle}>{selectedContext.listing.length} Entries</h3>
                <table className={commonStyles.table}>
                    <thead>
                        ...
                        <tr><td colSpan={4} className={commonStyles.tableHeadSpacer}></td></tr>
                    </thead>
                    <tbody className={styles.messagesBody}>
                        { sortBy(selectedContext.listing, message => message.ordinal)
        ...
        return (
            <tr key={message.id}>
                ...
                <td className={styles.payloadColumn}>
                    <CodeView language="json" code={this.prettyPrintJson(message.payload)} elementId={['debug', 'messages', message.id]} requestId={selectedContext.id} />
                </td>
            </tr>
        );
    }
