...
function addAgentDisplayProperties(filter) {
    switch (filter.agent) {
        case AgentType.Browser:
            return Object.assign({}, { displayName: 'Browser', icon: Icon.paths.Client, iconPathClassName: loggingFilterBarStyles.agentBrowserIcon }, filter);

        case AgentType.Server:
            return Object.assign({}, { displayName: 'Server', icon: Icon.paths.Server, iconPathClassName: loggingFilterBarStyles.agentServerIcon }, filter);
...
function addStatusCodeDisplayProperties(filter) {
    return Object.assign({}, { displayName: getStatusCodeShortNames(filter.statusCode), icon: getIconForStatusCode(filter.statusCode), iconClassName: styles.statusCodeIcon, iconPathClassName: getColorStyleForStatusCode(filter.statusCode) }, filter);
}

function mapStateToProps(state: IStoreState): IFilterBarProps {
    const statusCodeFilters = getStatusCodeFilterSummaries(state).map(addStatusCodeDisplayProperties);

    statusCodeFilters.sort((a, b) => b.statusCode - a.statusCode);

    return {
        groups: [
            { name: 'agent', filters: getAgentFilterSummaries(state).map(addAgentDisplayProperties) },
