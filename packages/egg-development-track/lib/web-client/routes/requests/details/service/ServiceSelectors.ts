...
import {
    convertStatusCodeToStatusCodeClass
} from './ServiceUtils';
import {
...
import {
    DataHttpRequestType,
    IDataHttpRequestPayload
} from 'modules/messages/schemas/IDataHttpRequestPayload';
import {
    DataHttpResponseType,
    IDataHttpResponsePayload
} from 'modules/messages/schemas/IDataHttpResponsePayload';
...
import { ITimelineAgentCount, ITimelineAgentSummary, AgentType } from '../timeline/TimelineInterfaces';
// --

/**
 * returns the current filters state for the by-agent filters
 */
const getAgentFiltersState = (state: IStoreState): IFilterValue => state.persisted.global.requests.details.service.filters.agent;

/**
 * returns the current filters state for the by-status-code filters
 */
const getStatusCodeFiltersState = (state: IStoreState): IFilterValue => state.persisted.global.requests.details.service.filters.statusCode;
...
export const getWebServiceExchanges: (state: IStoreState) => IExchangeModel[] = (function () {
    ...
    const getRawHttpData = createSelector(
        getSelectedContext,
        selectedContext => {
            const rawRequests = selectedContext ? getMessageByType<IDataHttpRequestPayload>(selectedContext.byType, DataHttpRequestType) : [];
            const rawResponses = selectedContext ? getMessageByType<IDataHttpResponsePayload>(selectedContext.byType, DataHttpResponseType) : [];
        ...
        (httpData, offsetFactor): IExchangeModel[] => {
            ...
            if (httpData.rawResponses) {
                httpData.rawResponses.forEach(response => {
                    const current: IExchangeModel = exchangesByCorrelationId[response.payload.correlationId];

                    if (current) {
                        current.response = response;
                        current.statusCodeClass = convertStatusCodeToStatusCodeClass(response.payload.statusCode);
                        current.duration = response.payload.duration;
                        current.linkedContextId = getValueAtKeyCaseInsensitive<string>(response.payload.headers, 'X-Glimpse-ContextID');
                    }
                    else {
                        const newExchange: IExchangeModel = {
                            ...
                            statusCodeClass: convertStatusCodeToStatusCodeClass(response.payload.statusCode),
            ...
            exchanges.forEach((exchange, index) => { exchange.index = index + 1; });
 ...
 */
export const getAgentFilteredtWebServiceExchanges: (state: IStoreState) => IExchangeModel[] = createSelector(
 ...
 */
export const getStatusCodeFilteredWebServiceExchanges: (state: IStoreState) => IExchangeModels = createSelector(
 ...
 */
export const getSelectedExchange: (state: IStoreState) => IExchangeModel = createSelector(
 ...
 */
export const getSelectedExchangeRequest = createSelector(
    getSelectedExchange,
    selectedExchange => {
 ...
 */
export const getSelectedExchangeResponse = createSelector(
    getSelectedExchange,
    selectedExchange => {
 ...
 */
export const getRequestHeadersSelector = createGetRequestHeadersSelector(getSelectedExchangeRequest);

/**
 * Selector to get unfiltered response headers for currently selected exchange
 */
export const getResponseHeadersSelector = createGetResponseHeadersSelector(getSelectedExchangeResponse);
 ...
 */
export const getFilteredResponseCookiesSelector = createGetFilteredResponseCookiesSelector(getSelectedExchangeResponse, getFilteredResponseHeadersSelector);
    ...
    return createSelector(
        ...
        (categoryCounts, categoryFilters: IFilterValue): IServiceStatusCodeClassSummary[] => {
            return categoryCounts
                .filter(e => e.statusCode !== StatusCodeClass.Other && e.statusCode !== StatusCodeClass.None)
                .map((entry) => {
                    return {
                        name: entry.name,
                        count: entry.count,
...
export const getTimelineEventsOffsetBoundary = createSelector(
    ...
    (exchanges: IExchangeModel[]) => {
        const maxOffset = exchanges.reduce((prev, exchange) => exchange.duration ? Math.max(prev, exchange.offset + exchange.duration) : prev, 0.1);
