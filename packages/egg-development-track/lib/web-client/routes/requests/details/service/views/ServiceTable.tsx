...
import { getStatusCodeFilteredWebServiceExchanges, getSelectedExchange, getTimelineEventsOffsetBoundary } from '../ServiceSelectors';
...
import { TimelineTable, ITimelineTableProps, ITimelineTableCallbacks, getActivityColumnPlaceholder } from 'common/components/timeline/TimelineTable';
...
const ServiceActivityBar =
    (props: { activity: IExchangeModel, metadata, totalDuration: number }) => {
...
const activityBarFunc = (activity: IExchangeModel, params, metadata) => {
    ...
    return <ServiceActivityBar activity={activity} metadata={metadata} totalDuration={totalDuration} />;
};
...
const activityFunc = (activity: IExchangeModel, minOffset: number, maxOffset: number, params) => {
    if (activity.response) {
        ...
        return <TimelineActivity activity={activity} activityBarFunc={activityBarFunc} minOffset={minOffset} maxOffset={maxOffset} metadata={metadata} params={params} />;
    }
    else {
...
const urlColumn = {
    ...
    valueFunc: (activity: IExchangeModel) => {
        const urlText = activity.request
            ? <UrlText alignHttpsIcon={true} url={activity.request.payload.url} protocol={activity.request.payload.protocol.identifier} />
        ...
        const link = activity.linkedContextId
            ? (
                <RouteButton
                    ...
                    <Icon
                        target={Icon.paths.Go}
                        className={styles.serviceTableLinkIcon}
                        pathClassName={styles.serviceTableLinkIconPath} />
                </RouteButton>
            )
            // To preserve alignment of URL text and links, return a placeholder <div> with the margin and width of the button and icon...
            : <div className={classNames(styles.serviceTableLinkButton, styles.serviceTableLinkIcon)} />;
...
const statusColumn = {
    ...
    valueFunc: (activity: IExchangeModel) => {
        return activity.response
            ? <StatusLabel statusCode={activity.response.payload.statusCode} statusMessage={activity.response.payload.statusMessage} />
            : undefined;
    },
    width: 9
};
...
const locationColumn = {
    headerFunc: () => 'Location',
    valueFunc: (activity: IExchangeModel) => {
        const payload = (activity.request && activity.request.payload) ? activity.request.payload : undefined;
...
function mapDispatchToProps(dispatch, props): ITimelineTableCallbacks {
    return {
        onSelectActivity: (activity: IExchangeModel) => {
            const requestId = activity.request ? activity.request.context.id : activity.response.context.id;
            const exchangeId = activity.eventId;

            dispatch(push(`/requests/${requestId}/${serviceTabName}/${exchangeId}`));
        }
    };
}
