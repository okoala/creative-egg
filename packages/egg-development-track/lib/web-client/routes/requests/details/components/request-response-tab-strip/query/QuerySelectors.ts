...
export function createGetQuerySelector(getRequestResponseMessgage: Reselect.Selector<IStoreState, IMessage<IRequestResponseQuery>>) {
    return createSelector(
        getRequestResponseMessgage,
        (message) => {
            if (message) {
                ...
                if (request && request.url) {
                    ...
                    if (parsedUrlQuery && parsedUrlQuery.length > 0) {
                        ...
                        const normalizedUrlQuery = parsedUrlQuery[0] === '?' ? parsedUrlQuery.slice(1) : parsedUrlQuery;
