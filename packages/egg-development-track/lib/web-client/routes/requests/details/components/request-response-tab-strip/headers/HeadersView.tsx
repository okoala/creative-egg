...
export class HeadersView extends React.Component<IHeadersProps, {}> {
    ...
    private renderSecurityRestricted() {
        ...
        if (isSecurityRestricted) {
            return <InformationLabel text="Incomplete headers list due to browser security restrictions." textClassName={styles.contentMessage} />;
        }
    }

    private renderTable(headers: { [key: string]: string[] }) {
        const flattenedHeaders = map(headers, (value: string[], key: string) => ({ name: trainCase(key), value: value }))
        ...
        return <FixedWidthLeftColumnTable className={styles.headersTable} columns={columnInfos} params={sortedHeaders} />;
    }
}
