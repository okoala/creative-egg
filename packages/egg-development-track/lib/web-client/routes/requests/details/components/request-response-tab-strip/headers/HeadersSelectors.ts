...
import { IRequestCookie, IResponseCookie, parseResponseCookies, parseRequestCookies, filterCookies, toCookieString, toSetCookieStrings } from '../cookies/CookieUtils';
...
export function projectHeadersToArray(headers: { [key: string]: string | string[] }):
    { [key: string]: string[] } {
    let projectedHeaders = mapValues(headers, value => isArray(value) ? value : [value]) as { [key: string]: string[] };
    return projectedHeaders;
}

export function filterGlimpseSpecificHeaders<T extends { [key: string]: string } | { [key: string]: string[] }>(headers: T):
    T {
    let filteredHeaders = pickBy(headers, (value, key) => {
...
export function createGetRequestHeadersSelector(getRequestMessage: Reselect.Selector<IStoreState, IMessage<IRequestHeaders>>):
    ...
    return createSelector(
        ...
        request => {
            return request
            ? {
                isSecurityRestricted: convertStringToAgentType(request.agent.source) === AgentType.Browser,
...
export function createGetFilteredRequestHeadersSelector(getRequestHeadersSelector: Reselect.Selector<IStoreState, IRequestHeadersResult>):
    Reselect.Selector<IStoreState, IRequestHeadersResult> {
    return createSelector(
        ...
        headerProps => {
            ...
            if (cookieKey && filteredHeaders[cookieKey]) {
                ...
                filteredHeaders[cookieKey][0] = toCookieString(filteredHeaders[cookieKey][0], filteredCookies);
            }
...
export function createGetResponseHeadersSelector(getResponseMessage: Reselect.Selector<IStoreState, IMessage<IResponseHeaders>>):
    ...
    return createSelector(
        ...
        response => {
            return response
                ? {
                    isSecurityRestricted: convertStringToAgentType(response.agent.source) === AgentType.Browser,
...
export function createGetFilteredResponseHeadersSelector(getResponseHeadersSelector: Reselect.Selector<IStoreState, IResponseHeadersResult>):
    Reselect.Selector<IStoreState, IResponseHeadersResult> {
    return createSelector(
...
export function createGetFilteredResponseCookiesSelector(getResponseMessage: Reselect.Selector<IStoreState, IMessage<IResponseHeaders>>, getFilteredResponseHeadersSelector: Reselect.Selector<IStoreState, IResponseHeadersResult>):
    Reselect.Selector<IStoreState, IResponseHeadersResult> {
    return createSelector(
        getResponseMessage,
