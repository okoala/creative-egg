...
import { IMultipartPart, IMultipartFilePart, IMultipartFormPart, MultipartType } from './BodyInterfaces';
...
const columns: IColumnInfo[] = [
            ...
            return param.isTruncated
                ? (
                    <div title="The content of this field has been truncated.">
                        <Icon className={styles.multipartSummaryTruncationIcon} pathClassName={styles.multipartSummaryTruncationIconPath} target={Icon.paths.Warning} />
                    </div>
                )
                : undefined;
        },
    ...
    },
    {
        ...
        valueFunc: (param: IMultipartPart) => {
            ...
            return (
                <span className={classNames({ [styles.multipartSummaryUndefinedContentType]: param.mediaType === undefined || param.mediaType.length === 0 })}>
                    { text }
                </span>
            );
        }
...
export const MultipartSummary = (props: { className?: string, parts: IMultipartPart[] }) => {
    ...
    return (
        <FixedWidthLeftColumnTable className={className} columns={columns} params={parts} />
    );
};
