...
import { BodyType, IRequestResponseBody, IRequestResponseBodyResult, IMultipartPart, IMultipartFilePart, IMultipartFormPart, MultipartType } from './BodyInterfaces';
import { ContentDispositionType, getContentDisposition, parseContentDisposition } from 'common/util/ContentDispositions';
...
function parseBodyPart(rawContent: string, rawSize: number, summary: IMultipartSummary): IMultipartPart {
    ...
    if (contentDisposition) {
        ...
        if (parsedContentDisposition && parsedContentDisposition.type === ContentDispositionType.FormData) {
...
function parseBodyParts(rawContent: string, rawSize: number, summaries: IMultipartSummary[]): IMultipartPart[] {
    return summaries
        .map(summary => parseBodyPart(rawContent, rawSize, summary))
        .filter(summary => summary !== undefined);
}

export function createGetBodySelector(getRequestResponseMessage: Reselect.Selector<IStoreState, IMessage<IRequestResponseBody>>):
    Reselect.Selector<IStoreState, IRequestResponseBodyResult> {
    return createSelector(
        getRequestResponseMessage,
        (message) => {
            if (message) {
