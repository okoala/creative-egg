...
import { FixedWidthLeftColumnTable, IColumnInfo } from 'common/components/FixedWidthLeftColumnTable';
...
function iconForBoolean(value: boolean): JSX.Element {
    let target = Icon.paths.CheckMark;
    let pathClassName = cookieStyles.iconCookieTrue;
    if (!value) {
        target = Icon.paths.BigX;
        pathClassName = cookieStyles.iconCookieFalse;
    }

    return (<Icon target={target} className={cookieStyles.icon} pathClassName={pathClassName} />);
}
...
function renderSecurityRestricted(isSecurityRestricted: boolean) {
    if (isSecurityRestricted) {
        return <InformationLabel text="Cookies not displayed due to browser security restrictions." textClassName={cookieStyles.contentMessage} />;
    }
}
        ...
        const columnInfos: IColumnInfo[] = [
            {
                header: 'Name',
                maxWidth: 200,
                valueFunc: (c: IRequestCookie) => { return c.name; },
                titleFunc: (c: IRequestCookie) => { return c.name; }
            },
            {
                header: 'Value',
                valueFunc: (c: IRequestCookie) => { return c.value; }
            },
            {
                header: 'Size',
                valueFunc: (c: IRequestCookie) => { return '' + c.size; }
            }
        ];

        return <FixedWidthLeftColumnTable className={styles.headersTable} columns={columnInfos} params={cookies} />;
    }
}
...
export class ResponseCookiesView extends React.Component<IResponseCookiesProps, {}> {
    ...
    private renderResponseCookiesTable(url: string, cookies: IResponseCookie[]) {
        ...
        const columnInfos: IColumnInfo[] = [
            {
                header: 'Name',
                valueFunc: (c: IResponseCookie) => { return c.name; },
                titleFunc: (c: IResponseCookie) => { return c.name; }
            },
            {
                header: 'Value',
                valueFunc: (c: IResponseCookie) => { return c.value; }
            },
            {
                header: 'Size',
                valueFunc: (c: IResponseCookie) => { return '' + c.size; }
            },
            {
                header: 'Domain',
                valueFunc: (c: IResponseCookie) => { return valueOrDefault(c.domain, defaultDomain); }
            },
            {
                header: 'Path',
                valueFunc: (c: IResponseCookie) => { return valueOrDefault(c.path, '/'); }
            },
            {
                header: 'Max-Age/Expires',
                valueFunc: (c: IResponseCookie) => { return valueOrDefault(c.maxAgeOrExpires, 'Session'); }
            },
            {
                header: 'HttpOnly',
                valueFunc: (c: IResponseCookie) => { return iconForBoolean(c.httpOnly); }, measureFunc: iconMeasureFunc
            },
            {
                header: 'Secure',
                valueFunc: (c: IResponseCookie) => { return iconForBoolean(c.secure); }, measureFunc: iconMeasureFunc
            },
            {
                header: 'SameSite',
                valueFunc: (c: IResponseCookie) => { return valueOrDefault(c.sameSite, '-'); }
            }
        ];
