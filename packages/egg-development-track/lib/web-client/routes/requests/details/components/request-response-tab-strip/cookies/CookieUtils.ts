...
export function filterCookies<T extends IRequestCookie | IResponseCookie>(cookies: T[]): T[]  {
    return cookies.filter(v => filteredCookieNames.indexOf(v.name) === -1);
}
...
export function parseRequestCookies(headers: { [key: string]: string }): IRequestCookie[] {
    ...
    if (cookieHeader) {
        ...
        while (end < cookieHeader.length) {
            ...
            if (start < end) {
                const name = eqIdx > start ? cookieHeader.substring(start, eqIdx).trim() : cookieHeader.substring(start, end).trim();
...
function parseResponseCookie(rawCookie: string): IResponseCookie {
...
export function parseResponseCookies(headers: { [key: string]: string | string[] }): IResponseCookie[] {
