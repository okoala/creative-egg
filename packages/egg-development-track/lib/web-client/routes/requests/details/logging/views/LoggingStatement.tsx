...
import styles from './LoggingStatement.scss';
...
emojione.regShortNames = new RegExp('<object[^>]*>.*?</object>|<a[^>]*>.*?</a>|<(?:object|embed|svg|img|div|p|a)[^>]*>|(' + shortnames + ')', 'gi');
            ...
            if (objectKey) {
                ...
                ReactDOM.render(
                    <Provider store={store}>
                        <JsonTree data={contentObj.objects[objectKey]} elementId={['logs', messageId, objectKey]} requestId={contextId} />
                    </Provider>,
                    node);
            }
        }
...
export default class LoggingStatement extends React.Component<ILoggingStatementProps, {}> {
    ...
    public componentDidMount() {
        ...
        if (content !== undefined || content !== null) { // tslint:disable-line:no-null-keyword
    ...
    public render() {
        ...
        if (content !== undefined || content !== null) { // tslint:disable-line:no-null-keyword
            return <span className={styles.statement} ref="target"></span>;
        }
        else {
            return undefined;
        }
