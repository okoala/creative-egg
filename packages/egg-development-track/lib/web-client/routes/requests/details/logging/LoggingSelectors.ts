...
import { CorrelationBeginType } from 'modules/messages/schemas/ICorrelationBeginPayload';
import { CorrelationEndType } from 'modules/messages/schemas/ICorrelationEndPayload';
import { ILogWritePayload, LogWriteType } from 'modules/messages/schemas/ILogWritePayload';
import { ILoggingMessage, LoggingMessageLevel, ILoggingAgentSummary, ILoggingLevelSummary, ILoggingLevelCount, ILoggingAgentCount } from './LoggingInterfaces';
...
import { getExpansionState, isExpanded } from 'routes/requests/components/expansion/ExpansionSelectors';
...
export const getLoggingMessages = (function () {
    ...
    function buildMessageModels(rawMessages: IMessage<ILogWritePayload & ICorrelationPayload>[], offsetFactor: number) {
        let currentOrdinal = 1;

        const allMessages = sortBy(rawMessages, [message => message.offset, message => message.ordinal])
            .map((message, index) => {
                let ordinal;
                if (message.types.indexOf('log-group-end') === -1) {
                    ordinal = currentOrdinal++;
                }
                const agent = isGroupMessage(message) ? AgentType.Other : normalizeAgent(message.agent.source);
    ...
    function matchCorrelatedMessage(messages) {
        const correlationMessages = getMessagesByType(messages, 'correlation');
        const begins = getMessagesByType(correlationMessages, CorrelationBeginType);
        const ends = getMessagesByType(correlationMessages, CorrelationEndType);
        ...
        begins.forEach((begin) => {
            ...
            if (allEnds && allEnds.length > 0) {
                ...
                const isGroup = end.types.indexOf('log-group-end') > -1;

                begin.correlations = { end, isBegin: true, isGroup, ends: allEnds };
            }
        });
        ...
        ends.forEach(end => {
            ...
            if (begin) {
                const isGroup = begin.types.indexOf('log-group-begin') > -1;
    ...
    function calculateGroupingState(messages: ILoggingMessage[], collapsedState: (message: ILoggingMessage) => boolean) {
        ...
        messages.forEach((message, x) => {
            ...
            let isVisible = !(message.correlations && message.correlations.isEnd && message.correlations.isGroup);
            if (isVisible) {
                    ...
                    if (frame.isActive) {
                        if (frame.isClosed) {
                            ...
                            if ((!message.correlations
                                ...
                                || (message.correlations && message.correlations.isGroup && frame.begin !== message.correlations.begin && message.correlations.end.index < frame.end.index))
                    ...
                    for (let i = groupingStack.length - 1; i > -1; i--) {
                        ...
                        if (message.payload.correlationId === frame.begin.payload.correlationId) {
                            ...
                            if (i === groupingStack.length - 1) {
                                ...
                                do {
                                    groupingStack.pop();
                                    frame = groupingStack.length ? groupingStack[groupingStack.length - 1] : undefined;
    ...
    return createSelector(
        ...
        (selectedContext, expansionState, offsetFactor): ILoggingMessage[] => {
            if (selectedContext) {
                const rawMessages = getMessageByType<ILogWritePayload & ICorrelationPayload>(selectedContext.byType, LogWriteType);
                ...
                const groupsCollapsed = (message: ILoggingMessage) => {
                    return !isExpanded(expansionState, ['logs', message.messageId], !message.payload.isCollapsed);
                };
...
const getLevelFiltersState = (state: IStoreState) => state.persisted.global.requests.details.logging.filters.level;

const getAgentFiltersState = (state: IStoreState) => state.persisted.global.requests.details.logging.filters.agent;

function filterLoggingRecords(messages: ILoggingMessage[], filter: (e: ILoggingMessage) => boolean): ILoggingMessage[] {
...
export const getCountableMessageCount = createSelector(
    getLoggingMessages,
    (messages): number => {
...
export const getLevelFiltersSummaries = (function () {
    const getByLevelCounts = createSelector(
        getAgentFilteredLoggingMessages,
