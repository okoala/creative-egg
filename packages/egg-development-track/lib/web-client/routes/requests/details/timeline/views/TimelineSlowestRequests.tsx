...
const TimelineSlowRequestView = (props: { span: ISlowTimelineSpan, slowness: number }) => {
    ...
    if (span) {
        ...
        if (span.isVisible) {
            ...
            eventName = (
                <div className={classNames(commonStyles.trimText, styles.timelineSlowRequestEventNoLink)} title={span.title}>
                    {span.title}
                </div>
            );
        }
        return (
            <div className={styles.timelineSlowRequestContainer}>
                <TimeDuration duration={span.duration} format={TimeDurationFormat.WholeMillesecondsFractionalSeconds} valueClassName={styles.timelineSlowRequestDuration} unitClassName={styles.timelineSlowRequestDurationUnits} />
                <div className={styles.timelineSlowRequestRow}>
                    <TimelineSlownessIcon className={styles.timelineSlowRequestRowIcon} slowness={slowness} />
                    <div>#{span.index}</div>
                </div>
                <div className={styles.timelineSlowRequestRow}>
                    <Icon target={Icon.paths.CategorySquare} className={styles.timelineSlowRequestRowIcon}
                        pathClassName={getColorStyleForCategory(span.category)} />
                    {eventName}
                </div>
            </div>
        );
        ...
        return (
            <div className={styles.timelineSlowRequestContainer}>
                ...
                <div className={styles.timelineSlowRequestRow}>
                    <Icon target={Icon.paths.Information} className={classNames(styles.timelineSlowRequestRowIcon, styles.timelineSlowRequestIconInformation)} />
    ...
    return (
        <div>
            ...
            <div className={classNames(className, styles.timelineSlowestRequestsContainer)}>
                <TimelineSlowRequestView span={spans[0]}
                    slowness={span0Slowness} />
                <TimelineSlowRequestView span={spans[1]}
                    slowness={span1Slowness} />
                <TimelineSlowRequestView span={spans[2]}
                    slowness={span2Slowness} />
            </div>
        </div>
    );
};
