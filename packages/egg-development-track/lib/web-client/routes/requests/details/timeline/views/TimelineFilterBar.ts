...
function addCateogryDisplayProperties(filter) {
    return Object.assign({}, { displayName: filter.name, icon: Icon.paths.CategorySquare, iconPathClassName: getColorStyleForCategory(filter.category) }, filter);
}
...
function mapStateToProps(state: IStoreState): IFilterBarProps {
    return {
        groups: [
            ...
            { name: 'category', filters: getCategoryFilterSummaries(state).map(addCateogryDisplayProperties) }
        ]
    };
}
