...
import { getSelectedTimelineEvents, getTotalTimelineDuration } from 'routes/requests/details/timeline/TimelineSelectors';
...
import { routeActivityAction, highlightOffsetsAction, resetHighlightedOffsetsAction } from 'routes/requests/details/timeline/TimelineActions';

import styles from './TimelineDetails.scss';
import commonStyles from 'common/components/Common.scss';

// tslint:disable-next-line:variable-name
const TimelineActivityBar =
    (props: { activity: ITimelineSpan, metadata }) => {
        const { activity, metadata } = props;

        return (
            <div className={classNames(styles.timelineDetailsActivityBar, getBackgroundStyleForCategory(activity.category))}>
                {
                    metadata
                    ? (
...
const TimelineMetadata =
    (props: { activity: ITimelineSpan, totalDuration: number}) => {
...
// tslint:disable-next-line:variable-name
const TimelineEvent =
    (props: { event: ITimelineEvent }) => {
        const { event } = props;
        const categoryText = event.customCategoryDescription || TimelineEventCategory[event.category];
        const title = `Event: ${event.title}\nCategory: ${categoryText}\nExecuted at: ${formatMilliseconds(event.offset).toString()}`;

        return (
            <div className={classNames(styles.timelineDetailsEvent, getBorderStyleForCategory(event.category))} title={title} />
        );
    };
...
const activityFunc = (activity: ITimelineSpan, minOffset: number, maxOffset: number, params: ITimelineDetailsParams) => {
    const metadata = <TimelineMetadata activity={activity} totalDuration={params.totalDuration} />;

    return <TimelineActivity activity={activity} activityBarFunc={activityBarFunc} minOffset={minOffset} maxOffset={maxOffset} metadata={metadata} />;
};
...
const indexColumn = {
    headerFunc: () => {
        return (
            <div className={styles.timelineDetailsIndexContainer}>
                ...
                <TimelineSlownessIcon className={styles.timelineDetailsSlownessIcon} slowness={undefined} />
                <AgentTypeIcon />
    ...
    valueFunc: (activity: ITimelineSpan, params: ITimelineDetailsParams) => {
        return (
            <div className={styles.timelineDetailsIndexContainer}>
                <TimelineSlownessIcon className={styles.timelineDetailsSlownessIcon} slowness={params.eventIDsToSlowness[activity.eventId]} />
...
const isActivityLinkable = (activity: ITimelineSpan): boolean => {
    // currently, only web service calls and middleware are linkable
    switch (activity.category) {
        case TimelineEventCategory.WebService:
            return true;

        case TimelineEventCategory.Request:
            return activity.source === AgentType.Server
                && activity.rawMessages.length > 0
                && 'correlationId' in activity.rawMessages[0].payload
                && 'types' in activity.rawMessages[0]
                && (activity.rawMessages[0].types.indexOf('middleware-start') !== -1 || activity.rawMessages[0].types.indexOf('middleware-end') !== -1);
...
function mapStateToProps(state: IStoreState, props): Partial<ITimelineTableProps<ITimelineDetailsParams>> {
    const { minOffset, maxOffset, spans, pointInTimeEvents, eventIDsToSlowness } = getSelectedTimelineEvents(state);
...
function mapDispatchToProps(dispatch, props): ITimelineTableCallbacks {
    return {
        onSelectActivity: (span: ITimelineSpan) => routeActivityAction(props.requestId, span)(dispatch),
        onHoverActivity: (span: ITimelineSpan) => {
            dispatch(highlightOffsetsAction({
