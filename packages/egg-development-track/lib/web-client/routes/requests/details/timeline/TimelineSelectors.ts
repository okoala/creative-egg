...
import { IDebugTimestampPayload, DebugTimestampType } from 'modules/messages/schemas/IDebugTimestampPayload';
import { IDebugTimestampMeasurementPayload, DebugTimestampMeasurementType } from 'modules/messages/schemas/IDebugTimestampMeasurementPayload';
...
import { getWebRequest, getWebResponse, getServerOffsetFactor, getBrowserNavigationTiming } from '../request/RequestSelectors';
import { getWebServiceExchanges } from '../service/ServiceSelectors';
import { IExchangeModel } from '../service/ServiceInterfaces';
import { getOperations, IRequestDetailDataOperationState } from '../data/DataSelectors';
import { getSelectedRequestPersistedState, getMessageByType } from 'routes/requests/RequestsSelector';
...
const sprintfjs = require('common/util/printf.js').default;
    ...
    (correlatedMiddleware): ITimelineSpan[] => {
        ...
        for (let i = 0; i < correlatedMiddleware.length; i++) {
            ...
            const e: ITimelineSpan = {
                ...
                index: 0, // index will get updated later after we sort all messages by ordinal
                title: `${middlewareName} Middleware`,
...
const getHttpRequestsTimelineEventsSelector = createSelector(
    ...
    (exchanges: IExchangeModel[]): ITimelineSpan[] => {
        ...
        for (let i = 0; i < exchanges.length; i++) {
            ...
            const title = exchange.request ? `Service ${exchange.request.payload.method}: ${exchange.request.payload.url}${titleStatus}` : 'Service';
            const frames = exchange.request && exchange.request.payload && exchange.request.payload.frames;
    ...
    (selecteContext: IContext): ITimelineSpan[] => {
        ...
        if (timingMessages) {
            ...
            if (timingMessages.payload.navigationStart && timingMessages.payload.domainLookupStart && timingMessages.payload.lookupDomainDuration) {
                const offset = timingMessages.payload.domainLookupStart - timingMessages.payload.navigationStart;
                const span: ITimelineSpan = {
            ...
            if (timingMessages.payload.navigationStart && timingMessages.payload.connectStart && timingMessages.payload.connectDuration) {
                const offset = timingMessages.payload.connectStart - timingMessages.payload.navigationStart;
                const span: ITimelineSpan = {
            ...
            if (timingMessages.payload.navigationStart && timingMessages.payload.secureConnectionStart && timingMessages.payload.connectEnd) {
                const offset = timingMessages.payload.secureConnectionStart - timingMessages.payload.navigationStart;
                const span: ITimelineSpan = {
            ...
            if (timingMessages.payload.navigationStart && timingMessages.payload.redirectStart && timingMessages.payload.redirectDuration) {
                const offset = timingMessages.payload.redirectStart - timingMessages.payload.navigationStart;
                const span: ITimelineSpan = {
...
const getDataTimelineEventsSelector = (function () {
    function getDataOperationTitle(op: IRequestDetailDataOperationState): string {
        // Data <operation> [<count> records] from [MongoDB|SQL] [(<database> .<table>)]
        let title = `Data ${op.operation}`;
        if (op.recordCount !== undefined && Number.isInteger(<number>op.recordCount)) {
            title = title + ` ${op.recordCount} `;
            title = title + (op.recordCount === 1 ? 'Record' : 'Records');
        }
        ...
        title = title + ` ${action} ${op.database}`;
        if (op.serverName && op.databaseName) {
            title = title + ` (${op.serverName}.${op.databaseName})`;
        }
        else if (op.serverName) {
            title = title + ` (${op.serverName})`;
        }
    ...
    return createSelector(
        getOperations,
        (dataOperations: IRequestDetailDataOperationState[]): ITimelineSpan[] => {
            const spans: ITimelineSpan[] = [];
...
const getDebugTimestampMessages = createSelector(
    getSelectedContext,
    (selecteContext: IContext): IMessage<IDebugTimestampPayload>[] => {
        return getMessageByType<IDebugTimestampPayload>(selecteContext.byType, DebugTimestampType);
    });

const getDebugTimestampMeasurementMessages = createSelector(
    getSelectedContext,
    (selecteContext: IContext): IMessage<IDebugTimestampMeasurementPayload>[] => {
        return getMessageByType<IDebugTimestampMeasurementPayload>(selecteContext.byType, DebugTimestampMeasurementType);
    });

const getDebugTimestampEventsSelector = createSelector(
    getDebugTimestampMessages,
    getServerOffsetFactor,
    (timestampMessages: IMessage<IDebugTimestampPayload>[], offsetFactor: number): ITimelineEvent[] => {
        return timestampMessages.map((m) => {
            const offset = (m.agent.source === 'server' ? offsetFactor : 0) + m.offset;
            return {
                eventId: m.id,
                ordinal: m.ordinal,
                title: `${m.payload.name} Mark`,
                offset: offset,
                sortOffset: offset,
                source: convertStringToAgentType(m.agent.source),
                category: TimelineEventCategory.Logs,
                customCategoryDescription: `Logs - ${LoggingMessageLevel[LoggingMessageLevel.Debug]}`,
                rawMessages: [m]
            } as ITimelineEvent;
        });
    });
...
const getDebugTimestampMeasurementSpansSelector = (function () {
    function correlateTimestampEvents(timestamps: ITimelineEvent[], measures: IMessage<IDebugTimestampMeasurementPayload>[]): ICorrelatedMessage<IDebugTimestampMeasurementPayload>[] {
        const byId: { [key: string]: ITimelineEvent } = {};

        // assumes that the message ID used as the timeline event ID.
        timestamps.forEach((m) => { byId[m.eventId] = m; });
    ...
    return createSelector(
        ...
        (timestampEvents: ITimelineEvent[],
            messages: IMessage<IDebugTimestampMeasurementPayload>[],
            offsetFactor: number): ITimelineSpan[] => {
            ...
            return correlated.map((m) => {
                ...
                return {
                    ...
                    index: 0, // index will get updated later after we sort all messages by ordinal
                    title: `${m.message.payload.name} Measure`,
        ...
        for (let i = 0; i < logMessages.length; i++) {
            if (logMessages[i].level === LoggingMessageLevel.Error) {
                ...
                if (Array.isArray(logMessages[i].payload.message)) {
                    ...
                    const result = sprintfjs(payload.message[0], payload.message.slice(1, payload.message.length), payload.tokenSupport);
                    title = result.formattedResult.innerText;
                }
                else if (typeof (logMessages[i].payload.message === 'string')) {
                    title = logMessages[i].payload.message;
                }
                ...
                const e: ITimelineEvent = {
                    ...
                    category: TimelineEventCategory.Logs,
                    customCategoryDescription: `Logs - ${LoggingMessageLevel[logMessages[i].level]}`,
                    rawMessages: []
                };
                events.push(e);
            }
...
export const getTimelineEvents = (function () {
    ...
    return createSelector(
        ...
        (webRequestSpans, webResponseSpans, middlewareSpans, httpSpans, networkTimingSpans, dataSpans, measureSpans, timestampEvents, consoleErrorEvents, networkTimingEvents): ITimelineEvents => {
            // sort by ordinal and number the events starting at 1
            const spans: ITimelineSpan[] = webResponseSpans.concat(middlewareSpans, httpSpans, networkTimingSpans, dataSpans, measureSpans);
            ...
            const pointInTimeEvents = webRequestSpans.concat(timestampEvents, consoleErrorEvents, networkTimingEvents);
...
export const getTimelineEventsOffsetBoundary = createSelector(
    ...
    events => {
        ...
        const maxActivityOffset = Math.max.apply(undefined, spans.map(span => span.offset + span.duration));
        const maxPointInTimeOffset = Math.max.apply(undefined, pointInTimeEvents.map(event => event.offset));
...
const getCategoryFiltersState = (state: IStoreState): ITimelineFilterValue => state.persisted.global.requests.details.timeline.filters.category;

const getAgentFiltersState = (state: IStoreState): ITimelineFilterValue => state.persisted.global.requests.details.timeline.filters.agent;

function filterTimelineEvents(events: ITimelineEvents, filter: (e: ITimelineEvent) => boolean): ITimelineEvents {
...
export const getOverviewTimelineEvents = createSelector(
...
export const getSelectedOffsetsForSelectedContext = createSelector(
    ...
    selectedRequestPersistedState => {
        ...
        if (!isNullOrUndefined(selectedOffsets.maxOffset) && selectedOffsets.maxOffset === selectedOffsets.minOffset) {
            selectedOffsets = Object.assign(selectedOffsets, { maxOffset: selectedOffsets.maxOffset + 1});
        }
...
export const getSelectedTimelineEvents = createSelector(
    ...
    (events, boundary, selectedOffsets): ISelectedTimelineEvents => {
        ...
        const minOffset = isNullOrUndefined(selectedOffsets.minOffset) ? boundary.minOffset : selectedOffsets.minOffset;
        const maxOffset =  isNullOrUndefined(selectedOffsets.maxOffset) ? boundary.maxOffset : selectedOffsets.maxOffset;

        const spans = events.spans.filter(span => span.offset <= maxOffset && (span.offset + span.duration) >= minOffset);
        const pointInTimeEvents = events.pointInTimeEvents.filter(event => event.offset <= maxOffset && event.offset >= minOffset);
...
export const getTotalTimelineDuration = createSelector(
    ...
    (events: ITimelineEvents) => {
        const maxSpanOffset = events.spans.reduce((previous, span) => Math.max(previous, span.offset + span.duration), 0);
        const maxEventOffset = events.pointInTimeEvents.reduce((previous, event) => Math.max(previous, event.offset), 0);
...
export const getSlowestTimelineEvents = (function () {
    function createSlowEvent(span: ITimelineSpan, selectedEvents: ISelectedTimelineEvents): ISlowTimelineSpan {
        if (!span) {
            return;
        }
        return Object.assign({
...
export const getUnfilteredByAgentCounts = createSelector(
    ...
    (events: ITimelineEvents): ITimelineAgentCount[] => {
        ...
        for (let i = 0; i < events.pointInTimeEvents.length; i++) {
            counts[events.pointInTimeEvents[i].source] = counts[events.pointInTimeEvents[i].source] + 1;
        }
...
function countByCategory(events: ITimelineEvents): ITimelineCategoryCount[] {
    ...
    for (let i = 0; i < events.pointInTimeEvents.length; i++) {
        counts[events.pointInTimeEvents[i].category] = counts[events.pointInTimeEvents[i].category] + 1;
    }

    return enumVals.map((v) => {
        return {
            name: v === TimelineEventCategory.WebService ? 'Web services' : TimelineEventCategory[v],
            count: counts[v],
            category: v
...
export const getUnfilteredByCategoryCounts = createSelector(
    getTimelineEvents,
    countByCategory
);
