...
import { getTimelineEventsOffsetBoundary, getOverviewTimelineEvents, getSelectedOffsetsForSelectedContext } from '../TimelineSelectors';
import { getSelectedContextId, getSingleMessageByType} from 'routes/requests/RequestsSelector';
import { getBrowserNavigationTimingOffsets, IBrowserNavigationTimingOffsets, BrowserNavigationTimingSegments } from 'routes/requests/RequestsOverviewSelector';
...
const browserNavigationTimingLabels: IBrowserNavigationTimingMap = {
    Reset: 'Show all',
    Custom: 'Custom',
...
export class TimelineOverviewView extends React.Component<ITimelineOverviewProps & ITimelineOverviewCallbacks, {}> {
    ...
    private renderMessage() {
        ...
        if (wasTruncated) {
            const text = 'Some shorter-duration events are hidden. You can still find all events in the table below.';

            return (
                <div title={text} className={styles.timelineOverviewMessage}>
                    <Icon target={Icon.paths.Warning} className={styles.timelineOverviewMessageIcon} pathClassName={styles.timelineOverviewMessageIconPath} />
    ...
    private handleSelectRangeChange = (e: React.ChangeEvent<HTMLSelectElement>): void => {
        ...
        switch (value) {
            ...
            default:
                ...
                const { offsets } = browserNavigationTimingOffsets[value];
    ...
    private renderSelectRange() {
        ...
        return (
            <div className={styles.timelineOverviewSelect} tabIndex={-1}>
                <select
                    className={styles.timelineOverviewSelectSelect}
                    ...
                    {getNamesForEnum(BrowserNavigationTimingSegments).map(key =>
                        <option key={key} value={BrowserNavigationTimingSegments[key]} disabled={navigationOptionsDisabled}>
                            {browserNavigationTimingLabels[key]}
                        </option>
                    )}
                </select>
                <div className={styles.timelineOverviewSelectHeader}>
                    { this.getSelectHeaderName(selectedSegment) }
                    <div className={styles.timelineOverviewSelectHeaderArrow} />
                </div>
            </div>
        );
    }
    ...
    private getSelectHeaderName(selectedSegment: string): string {
        let headerName = BrowserNavigationTimingSegments[selectedSegment];

        // handle `Custom` and `Show all` cases
        headerName = (selectedSegment === '') ? 'Custom' : headerName;
        headerName = (selectedSegment === 'reset') ? 'Reset' : headerName;

        return browserNavigationTimingLabels[headerName];
    }
    ...
    private renderHeaders() {
        ...
        return (
            <div className={styles.timelineOverviewHeaders}>
                {
                    range(1, offsetTicks + 1)
                        .map(index => (
                            <div key={index.toString()} className={styles.timelineOverviewHeader}>
                                <TimeDuration duration={minOffset + (index * tickValue)} format={TimeDurationFormat.WholeMillesecondsFractionalSeconds} />
                            </div>))
                }
            </div>
        );
    ...
    private renderCells() {
        ...
        return (
            <div className={styles.timelineOverviewCells}>
                {
                    range(1, offsetTicks + 1)
                        .map(index => <div key={index.toString()} className={styles.timelineOverviewCell} />)
                }
            </div>
        );
    }
    ...
    private renderActivities() {
        ...
        return (
            <div className={styles.timelineOverviewActivities}>
                {spans.map(activity => <TimelineActivity key={activity.eventId} activity={activity} activityBarFunc={TimelineOverviewView.renderActivity} minOffset={minOffset} maxOffset={maxOffset} />)}
            </div>
        );
    }

    private static renderActivity(activity: ITimelineSpan, params, metadata) {
        return <div className={classNames(styles.timelineOverviewActivity, getBackgroundStyleForCategory(activity.category))} />;
    }
    ...
    private renderEvent(event: ITimelineEvent) {
        ...
        return (
            <div key={event.eventId} style={containerStyle}>
                <div className={classNames(styles.timelineOverviewEvent, getBorderStyleForCategory(event.category))} />
            </div>
        );
    }
    ...
    private renderHighlightLayer() {
        ...
        const clampedHighlightedMinOffset = Math.max(minOffset, Math.min(maxOffset, highlightedMinOffset || minOffset));
        const clampedHighlightedMaxOffset = Math.min(maxOffset, Math.max(minOffset, highlightedMaxOffset || maxOffset));
    ...
    private renderSelectionLayer() {
        ...
        const effectiveSelectedMinOffset = Math.max(minOffset, Math.min(maxOffset, selectedMinOffset || minOffset));
        const effectiveSelectedMaxOffset = Math.min(maxOffset, Math.max(minOffset, selectedMaxOffset || maxOffset));
        ...
        return (
            <div className={styles.timelineOverviewSelectionLayer}>
                <div ref={gripContainer => this.gripContainer = gripContainer} style={containerStyle}>
                    <div style={leftMarginStyle}>
                        <div className={styles.timelineOverviewUnselected} />
                    </div>
                    <div ref={leftGrip => this.leftGrip = leftGrip} className={styles.timelineOverviewGripContainer} onMouseDown={this.onMouseDown}>
                        {this.renderGrip(/* isLeft: */ true)}
                    </div>
                    <div style={barMarginStyle}>
                        {this.renderSelection()}
                    </div>
                    <div ref={rightGrip => this.rightGrip = rightGrip} className={styles.timelineOverviewGripContainer} onMouseDown={this.onMouseDown}>
    ...
    private renderGrip(isLeft: boolean) {
        return (
            <div className={styles.timelineOverviewGrip}>
                <div className={isLeft ? styles.timelineOverviewGripHandleLeft : styles.timelineOverviewGripHandleRight}>&nbsp;</div>
                <div className={isLeft ? styles.timelineOverviewGripLeft : styles.timelineOverviewGripRight} />
            </div>
        );
    }
    ...
    private onMouseDown(e: React.MouseEvent<HTMLDivElement>) {
        ...
        const grip = dragging === DraggingState.DraggingLeft
            ? this.leftGrip
            : this.rightGrip;
        ...
        const minLeft = dragging === DraggingState.DraggingLeft
            ? 0
            : leftGripRight + 1;
...
function mapStateToProps(state: IStoreState, props): ITimelineOverviewProps {
    ...
    const browserNavigationTiming = getSingleMessageByType<IBrowserNavigationTimingPayload>(context.byType, BrowserNavigationTimingType);
    const browserNavigationTimingOffsets = browserNavigationTiming && browserNavigationTiming.payload
