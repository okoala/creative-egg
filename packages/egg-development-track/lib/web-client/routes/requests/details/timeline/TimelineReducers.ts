...
import { ITimelineFilterValue, ITimelineSelectedOffsetsState, ITimelineOffsetsActionConfig } from './TimelineInterfaces';
import { resetAllAction, toggleCategoryAction, toggleAgentAction, selectOffsetsAction, resetOffsetsAction, highlightOffsetsAction, resetHighlightedOffsetsAction } from './TimelineActions';
import { getIntegersForEnum } from 'common/util/CommonUtilities';
import { Action, combineReducers } from 'redux';

function toggleFilter(filtersState: ITimelineFilterValue, targetFilter: AgentType | TimelineEventCategory): ITimelineFilterValue {
...
function showAll(filtersState: ITimelineFilterValue, enumType: {}): ITimelineFilterValue {
    ...
    nums.forEach((v) => { allVisible = allVisible && filtersState[v]; });
    if (!allVisible) {
        const updatedFiltersState = {};
        nums.forEach(v => { updatedFiltersState[v] = true; });
        return updatedFiltersState;
    }
    return filtersState;
}
...
export function categoryFiltersReducer(state: ITimelineFilterValue = defaultCategoryFiltersState, action: Action) {
    switch (action.type) {
...
export function agentFiltersReducer(state: ITimelineFilterValue = defaultAgentFiltersState, action: Action) {
    switch (action.type) {
...
function selectOffsets(state: ITimelineSelectedOffsetsState, data: ITimelineOffsetsActionConfig): ITimelineSelectedOffsetsState {
...
function highlightOffsets(state: ITimelineSelectedOffsetsState, data: ITimelineOffsetsActionConfig): ITimelineSelectedOffsetsState {
...
function resetHighlightedOffsets(state: ITimelineSelectedOffsetsState): ITimelineSelectedOffsetsState {
    return {
        ...state,
...
export function selectedOffsetsReducer(state: ITimelineSelectedOffsetsState = initialTimelineSelectedOffsetsState, action: Action): ITimelineSelectedOffsetsState {
    switch (action.type) {
