...
import { BrowserNavigationTimingType, IBrowserNavigationTimingPayload } from 'client/modules/messages/schemas/IBrowserNavigationTimingPayload';
...
export class RequestsSideBarRequest extends React.Component<IRequestsSideBarRequestProps & IRequestsSideBarRequestCallbacks & IConnectedRequestsSideBarRequestProps, {}> {
    constructor(props) {
        super(props);
            ...
            <RouteButton
                ...
                <div className={styles.requestContainer}>
                    <div className={classNames(styles.requestSummaryLine)}>
                        <div className={commonStyles.trimText}><UrlText suppressColor={isSelected} url={url} protocol={protocol} /></div>
                    ...
                    <div className={classNames(styles.requestDetailLine)}>
                        <div className={styles.requestStatusContainer}>
                            <ClientLabel userAgent={userAgent} className={styles.requestClientIcon} showIconOnly={true} />
                            <span>{method}</span> <StatusLabel className={styles.statusLabel} statusCode={statusCode} statusMessage={statusMessage} />
                        </div>
...
function mapStateToProps(state: IStoreState, ownProps: IConnectedRequestsSideBarRequestProps): IRequestsSideBarRequestProps {
    const { request, selectedRequestId } = ownProps;
    const { webRequest, webResponse, id } = request;

    const browserNavigationTimingMessage = getSingleMessageByType<IBrowserNavigationTimingPayload>(request.context.byType, BrowserNavigationTimingType);
...
export default connect(mapStateToProps, mapDispatchToProps)(RequestsSideBarRequest) as React.ComponentClass<IConnectedRequestsSideBarRequestProps>;
