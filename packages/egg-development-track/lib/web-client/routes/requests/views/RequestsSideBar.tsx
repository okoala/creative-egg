...
export class RequestsSideBar extends React.Component<IRequestsSideBarProps & IRequestsSideBarCallbacks, {}> {
    ...
    public render() {
        const { requestFilterDetails, selectedRequestId } = this.props;
        const {
            listing,
            totalCount,
            filteredCount
        ...
        return (
            <SideBar
                title={`Requests (${filteredCount})`}
                rightButtonTitle="Clear all"
                rightButtonOnClick={this.props.clearAll}>
                <SideBarSection
                    title={`History (${RequestsSideBar.filterTextCount(totalCount, filteredCount)})`}>
                    <div className={styles.historyContainer}>
                        <DataLoadingTimer
                            ...
                            getLoadingStatusBlock={this.renderLoading}>
                                {RequestsSideBar.renderRequests(listing, selectedRequestId)}
                        </DataLoadingTimer>
                    </div>
                </SideBarSection>
            </SideBar>
    ...
    private static renderRequests(listing: IRequest[], selectedRequestId: string): JSX.Element[] {
        return listing
            .map(request => <RequestsSideBarRequest selectedRequestId={selectedRequestId} request={request} key={request.id} />)
            .reverse();
    }

    private static filterTextCount(totalCount: number, filteredCount: number): string | number {
        return filteredCount;

        // TODO: add back in once filtering is enabled
        ...
        if (requestFilterDetails.listing.length && !selectedRequestId) {
            ...
            if (new Date().getTime() - followLoadTime < 5000) {
                ...
                for (let i = requestFilterDetails.listing.length - 1; i > -1; i--) {
                    ...
                    if (request.id === selectedRequestId || request.id === followLastBlacklistedRequestId) {
                        break;
                    }
                    // if the request is followable and we are allowed to follow
                        ...
                        const browserNavigationTiming = getSingleMessageByType<IBrowserNavigationTimingPayload>(request.context.byType, BrowserNavigationTimingType);
...
export interface IConnectedRequestsSideBarProps {
    /**
     * Detials of the current filter that is in place
     */
    requestFilterDetails: IRequestFilterDetails;
...
export default connect(undefined, mapDispatchToProps)(RequestsSideBar) as React.ComponentClass<IConnectedRequestsSideBarProps>;
