...
const greetMessage = (version) => {
    if (browserSupportsLogStyles() && isEdge() === false) {
        ...
        const args = [
            `\n%c%c%c${NAME} ◦ ${version} ◦%cIssues/Feedback?%chttps://github.com/Glimpse/Home/issues/new%c♥%c\n\n`,
            ...
            `color: ${DEEP_GRAY}; background: ${GRAY}; padding: ${VERT_PADDING}px ${VERT_PADDING  / 1.2}px ${VERT_PADDING}px 0; ${FONT} border-top: 1px solid ${DEEP_GRAY}; border-bottom: 1px solid ${DEEP_GRAY};`, // link
            `color: ${DEEP_PINK}; background: ${DEEP_GRAY}; padding: ${VERT_PADDING + 1}px 10px; ${FONT}`, // heart
        ...
        console.log(`\n`);
        console.log(`⚡ ${NAME} ◦ v${version} ⚡ Issues/Feedback? ↪ https://github.com/Glimpse/Home/issues/new ↩ ♥`);
        console.log(`\n`);
    }
};
