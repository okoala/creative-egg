...
*/
export default class DataLoadingErrorTimer extends React.Component<IRequestsDataLoadingTimerProps, IRequestsDataLoadingTimerState> {

    public static defaultProps = {
        getErrorStatusBlock: () => { return 'Error.'; },
    ...
    private handleErrorCase(): JSX.Element | string {
        const {
            isErrorBeforeTimeout,
            getErrorStatusBlock,
            getLoadingStatusBlock
        } = this.props;

        // error matters only when timeout elpased - show `loading` until
        ...
        return (this.state.isTimeoutElapsed || isErrorBeforeTimeout)
                    ? getErrorStatusBlock() : getLoadingStatusBlock();
    }
}
