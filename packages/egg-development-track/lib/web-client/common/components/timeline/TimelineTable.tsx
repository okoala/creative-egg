...
export interface ITimelineTableProps<T> {
     ...
     */
    activityFunc: (activity: ITimelineComponentSpan, minOffset: number, maxOffset: number, params: T) => JSX.Element;
...
export interface ITimelineTableCallbacks {
     ...
     */
    onSelectActivity?: (activity: ITimelineComponentSpan) => void;
...
export class TimelineTable<T> extends React.Component<ITimelineTableProps<T> & ITimelineTableCallbacks, {}> {
    private TimelineDetailsActivityRow = // tslint:disable-line:variable-name
        class TimelineTableActivityRow extends React.Component<ITimelineTableActivityRowProps<T> & ITimelineTableActivityRowCallbacks, {}> {
            public static createBlankRow<T>(columns: ITimelineTableColumn<T>[], offsetTicks: number) {
                return (
                    <tr key="blank">
                        {
                            columns.map((column, index) => <td className={classNames(column.valueClassName)} key={index.toString()}>&nbsp;</td>)
                        }
            ...
            private onClick = (e) => {
                const { activity, onSelectActivity } = this.props;
            ...
            public render() {
                const { activity, columns, params, rowClassName, selectedActivity, selectedRowClassName } = this.props;
            ...
            private renderItems(columns: ITimelineTableColumn<T>[], activity: ITimelineComponentSpan, params: T) {
    ...
    public componentDidUpdate() {
        const { selectedActivity, onSelectActivity, shouldAutoSelectFirstActivity, spans } = this.props;
        this.validateProps();

        if (shouldAutoSelectFirstActivity && !selectedActivity && spans.length) {
            onSelectActivity(spans[0]);
        }
    }
                    ...
                    <div className={styles.timelineTableHeaderTable}>
                        <table className={styles.timelineTableTable}>
                            <thead>
                                ...
                                <tr>
                                    <td colSpan={columns.length} className={commonStyles.tableHeadSpacer} />
                                </tr>
                            </thead>
                        </table>
                    </div>
                ...
                <div className={styles.timelineTableTableContainer}>
                    <div className={styles.timelineTableScrollContainer}>
                        <div className={styles.timelineTableEventsContainer}>
                            <div className={styles.timelineTableIntervalContainer} style={{ width: activityWidth, right: activityRightShift }}>
                                {
                                    range(1, offsetTicks + 1).map(index =>
                                        <div className={styles.timelineTableInterval} key={index.toString()} />)
                                }
                            </div>
                            <table className={styles.timelineTableTable}>
                                <thead>
                            ...
                            </table>
                            {
                                ...
                                ? (
                                    <div className={styles.timelineTableEvents} style={{ width: activityWidth, right: activityRightShift }}>
    ...
    private renderTableHead(columns: ITimelineTableColumn<T>[], activityWidth: string) {
        return columns.map((column, index) => {
            return (
                <th className={classNames(column.headerClassName)} key={`column-${index}`} width={`${column.width}%`}>
    ...
    private renderTableBodyHead(columns: ITimelineTableColumn<T>[], activityWidth: string) {
        return columns.map((column, index) => {
            return (
                <th className={column.headerClassName} key={`column-${index}`} width={`${column.width}%`} />
            );
        });
    }
