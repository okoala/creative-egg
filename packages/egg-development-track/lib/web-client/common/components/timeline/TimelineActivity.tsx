...
function getMetadataSide(leftSize: number, barSize: number, rightSize: number, metadata): MetadataSide {
    if (metadata) {
...
const TimelineActivity = (props: ITimelineActivityProps) => {
    ...
    const activityEndOffset = Math.min(maxOffset, Math.max(minOffset, activity.offset + activity.duration));
            ...
            <div style={barMarginStyle}>
                {
                    activityBarFunc(
                        activity,
                        params,
                        metadataSide === MetadataSide.Inline
                            ? metadata
                            : undefined)
                }
            </div>
