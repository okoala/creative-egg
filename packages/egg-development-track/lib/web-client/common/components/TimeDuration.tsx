...
export function formatMilliseconds(ms: number, format?: TimeDurationFormat): TimeDurationValue {
    ...
    const hasFractionalMilliseconds = effectiveFormat === TimeDurationFormat.FractionalMillesecondsFractionalSeconds;
...
export class TimeDuration extends React.PureComponent<ITimeDurationProps, {}> {
    public render() {
        ...
        if (duration !== undefined) {
            ...
            return <div><span className={valueClassName}>{formattedValue.value}</span><span className={unitClassName}>&nbsp;{formattedValue.unit}</span></div>;
        }
        else {
            return <span>-</span>;
        }
