...
export class FixedWidthLeftColumnTable extends React.Component<ITableProps, ITableState> {
    ...
    constructor(props) {
        ...
        this.onTableMeasure = this.onTableMeasure.bind(this);
    }
    ...
    public render() {
                    ...
                    {columns.map((column, columnIndex) => {
                        return (
                            <Column
                                ...
                                cell={(props) => {
                                    const title = column.titleFunc ? column.titleFunc(params[props.rowIndex]) : '';
                                ...
                                fixed={column.isFixed !== undefined ? column.isFixed : (columnIndex === 0)}
                                isResizable={true}
        ...
        return (
            <Measure onMeasure={this.onTableMeasure}>
                <div className={classNames(styles.fixedWidthLeftColumnTableContainer, className)}
    ...
    private setColumnWidths(columns: IColumnInfo[], params: Object[]) {
        ...
        for (let columnIndex = 0; columnIndex < columns.length; columnIndex++) {
            ...
            if (column.measureFunc) {
                widestColumnCell =
                    params.map(param => column.measureFunc(param))
                          .reduce((prev, paramWidth) => Math.max(prev, paramWidth), 0);
            }
            else {
                const longestColumnCell =
                    params.map(param => (column.textFunc ? column.textFunc : column.valueFunc)(param) as string)
                          .reduce((prev, value) => value && (value.length > prev.length) ? value : prev, '');
    ...
    private static getTextWidth(text: string): number {
        return measureTextWidth({
            text,
            size: '13px'
        }) + 24; // We need to account for padding, so we add some extra
    }
