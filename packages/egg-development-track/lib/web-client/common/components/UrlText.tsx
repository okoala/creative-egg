...
export class UrlText extends React.PureComponent<IUrlTextProps & IConnectedUrlTextProps, {}> {
    ...
    public render() {
        const { alignHttpsIcon, url, displayUrl, showHttpsIcon, showHttpsText, suppressColor } = this.props;

        const colorCssClass = suppressColor ? '' : styles.urlProtocolHttpsColor;

        const httpsIcon = showHttpsIcon
            ? <Icon target={Icon.paths.Lock} className={classNames(styles.urlProtocolHttps, colorCssClass)} />
            : alignHttpsIcon
                ? <Icon target={undefined} className={styles.urlProtocolHttps} />
                : undefined;

        const httpsText = showHttpsText ? <span className={classNames(styles.urlProtocol, styles.urlProtocolHttps, colorCssClass)}>{'https://'}</span> : undefined;

        return (
            <div className={classNames(styles.url, commonStyles.trimText)} title={url}>
...
export function mapStateToProps(state: IStoreState, ownProps: IConnectedUrlTextProps): IUrlTextProps {
