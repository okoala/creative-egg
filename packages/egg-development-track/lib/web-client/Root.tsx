...
const requestsConfig = require('./routes/requests/requests-config').default; // tslint:disable-line:no-var-requires
...
export class Root extends React.Component<IShellViewProps, {}> {
    public render() {
        ...
        const history = syncHistoryWithStore(browserHistory, store, { selectLocationState: (state: IStoreState) => state.session.routing });
        const rootRoute = {
            childRoutes: [{
                path: '/',
                component: ShellView,
                ...
                onEnter(nextState: RouterState, replace: RedirectFunction) {
                    ...
                    if (location.query[baseUrlProperty] || location.query[metadataUriProperty]) {
                        ...
                        const newQuery = omit(location.query, [baseUrlProperty, metadataUriProperty]);
