...
import * as historyMessages from './messages-actions-history';
import * as liveMessages from './messages-actions-live';
import * as localMessages from './messages-actions-local';
...
export function fetchByContext(contextId) {
    return dispatch => {
        ...
        dispatch(liveMessages.subscribeByContext(contextId, results => fetchByContextReceived(dispatch, contextId, results, 'live')));
        dispatch(historyMessages.fetchByContext(contextId, results => fetchByContextReceived(dispatch, contextId, results, 'history')));
    };
}
