...
export class ConfigSettings extends events.EventEmitter implements IConfigSettings {
   ...
   *
   * @commandLineArgs - an object containing key/value pairs of command-line args,
   * or a string which specifies the variable
   ...
   */
  public constructor(
    commandLineArgs: object | string,
    environmentVariables: object | string,
    ...
    if (typeof commandLineArgs === 'string') {
      commandLineArgs = ConfigSettings.filterCommandLineArgs(commandLineArgs as string, process.argv);
    } else if (typeof commandLineArgs !== 'object') {
      commandLineArgs = {};
    }

    if (typeof environmentVariables === 'string') {
      environmentVariables = ConfigSettings.filterEnvironmentArgs(environmentVariables as string, process.env);
