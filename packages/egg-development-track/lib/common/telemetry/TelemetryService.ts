...
export class TelemetryService implements ITelemetryService {
  ...
  // telemetry configuration details retrieved from the glimpse server.
  // This is populated asynchronously, so there's logic to
  ...
  /**
   * send an event (if app insights is currently configured),
   * or queue it for sending later when app insights is configured.
   */
  public sendEvent(eventName: string, properties: IProperties, measurements: IMeasurements) {
    if (this.isTelemetryEnabled && this.aiClient) {
      ...
      let cp = properties as ICommonProperties;
      if (!cp.sessionId || !cp.glimpseComponentType) {
        ...
        cp = newProps as ICommonProperties;
