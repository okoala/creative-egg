...
export class RequireAnalyzer {
   ...
   */
  public static getReferencedPackageNames(
    nodeModules: { [key: string]: NodeModule },
    excludedPackages: IKnownPackage[],
  ): string[] {
    const excludedPackageNames = [RequireAnalyzer.applicationPackageName].concat(
      excludedPackages.map((parentModule) => parentModule.packageName)
    );
   ...
   */
  public static getReferencedPackageName(
    nodeModule: NodeModule,
    knownModules: IKnownPackage[] = [],
    parentModuleCache: { [key: string]: string } = {},
  ): string {
    if (nodeModule) {
