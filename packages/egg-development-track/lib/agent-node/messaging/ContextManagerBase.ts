...
import { IErrorReportingService, createNoContextError, createUnexpectedContextError } from '../../common';
      ...
      if (currentContextID !== expectedContextID) {
        if (this.errorReportingService) {
          this.errorReportingService.reportError(
            createUnexpectedContextError(), location, expectedContextID, currentContextID,
          );
        }
      }
