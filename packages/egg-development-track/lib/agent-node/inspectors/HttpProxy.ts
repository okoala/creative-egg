...
export class HttpProxy extends ProxyBase {
  ...
  private raiseRequestStartEvent(
  ...
  // tslint:disable-next-line:no-any
  private raiseResponseStartEvent(
    req: http.IncomingMessage,
    res: http.ServerResponse,
    ...
    httpModule.createServer = function createServer(options, cb, ...args) {
      function internalCallback(req: http.IncomingMessage, res: http.ServerResponse, ...rest) {
        ...
        agent.providers.contextManager.runInNewContext(req, (context: IContext) => {
          ...
          let isMultiPartFormData: boolean;
          let multiPartFormSummarizer: IMultiPartFormSummarizer;
          ...
          req.on('end', () => {
            ...
            if (!requestEndSent) {
              self.raiseRequestEndEvent(
                req, res,
                requestBodyChunks,
                requestBodyLength,
                requestStartTime,
                multiPartFormSummarizer ? multiPartFormSummarizer.getParts() : [],
              );
              requestEndSent = true;
            }
          });
          ...
          res.setHeader = function setHeader(name, value, ...setHeaderArgs) {
            ...
            if (
              ...
              value.indexOf('text/html') === 0 &&
              RequestHelper.header(req, IS_AJAX_HEADER) !== 'true'
            ) {
