...
export class ExpressInspectorMiddleware {
  ...
  public init(broker: IAgentBroker, providers: IAgentProviders) {
    ...
    this.middlewareWrapper = new MiddlewareWrapper(
      this.stackHelper, providers.messageConverter, this.broker, this.contextManager, this.dateTime,
    );

    Tracing.onAlways(EVENT_EXPRESS_INVOKE_PRE_ROUTE_METHOD, (event) => {
      this.onMiddlewareMethodInvoked(
        event.data.originalThis, event.data.originalArgs, event.data.methodName, event.data.proxyFunction,
      );
    });
  ...
  private onMiddlewareMethodInvoked(originalThis, originalArgs, method, proxyFunction) {
    ...
    if (originalArgs && originalArgs.length >= 1) {
      ...
      this.wrapNestedMiddleware(
        originalArgs, [originalThis.path], method !== 'all' ? method.toUpperCase() : undefined, registrationCallstack,
      );
    }
  }
      ...
      if (originalArgs.length > 1) {
        ...
        if (typeof middleware === 'function') {
          if (!originalThis.glimpse) {
            ...
            if (
              originalThis.glimpse.firstUse.name === 'query' && originalThis.glimpse.secondUse.name === 'expressInit'
            ) {
              MiddlewareWrapper.attachMetadata(
                originalThis.glimpse.firstUse, 'query', 'Express Query Parser', 'express',
              );
              MiddlewareWrapper.attachMetadata(
                originalThis.glimpse.secondUse, 'expressInit', 'Express Initialization', 'express',
              );
            }
          }
        }
