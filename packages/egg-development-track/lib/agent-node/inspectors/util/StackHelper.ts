...
// note:  this interface is consistent with the stack frame interface from
// https://github.com/stacktracejs/error-stack-parser/blob/master/error-stack-parser.d.ts
// the intention was to use the stack-frame GPS module and our own parsing logic that runs ~2x faster than
// stackframejs stack frame parsing.  However,
// there are errors about xmlhttprequest being undefined in stackframe-gps.
// These can possibly be worked around, so we'll maintain a consistent
...
export class StackHelper implements IStackHelper {
  ...
  public captureStack(stackTopFunction, numFrames: number): IStackFrame[] {
    ...
    if (numFrames > 0) {
      ...
      const saveLimit = (Error as any).stackTraceLimit;
      (Error as any).stackTraceLimit = numFrames;
      (Error as any).captureStackTrace(obj, stackTopFunction);
      (Error as any).stackTraceLimit = saveLimit;
      ...
      for (let i = 1; i < frames.length; i++) {
        if (frames[i]) {
          const frame = this.parseFrameEntry(frames[i]);
    ...
    if (frames && frames.length > 0) {
      ...
      for (let i = 0; i < frames.length; i++) {
        if (frames[i].fileName &&
          !this.isBuiltIn(frames[i]) &&
          !this.isGlimpseFrame(frames[i]) &&
          frames[i].fileName !== 'native' &&
          frames[i].fileName.indexOf(nodeModules) === -1) {
          loc = frames[i];
          break;
        }
      }
  ...
  public getMappingUriFromContent(sourceFile: string, data: string): url.Url {
    ...
    // need to have the string concat below,
    // without it will break remap-istanbul when it tries to use source maps on this file
    const sourceMapMark = '\n//# ' + 'sourceMappingURL=';
  ...
  /**
   * given the existing stack frame,
   * create map a new stack frame where data has been replaced with values from source maps, if available.
   */
  ...
  private createMappedFrame(sourceMapConsumer, frame: IStackFrame): IStackFrame {
    if (sourceMapConsumer) {
      ...
      const validPosition: boolean = position &&
        position.source && (position.line || position.line === 0) && (position.column || position.column === 0);
      if (validPosition) {
        frame = {
  ...
  private loadSourceMapConsumer(generatedFile: string, uri: url.Url, callback: (sourceMapConsumer) => void) {
    if (uri && (!uri.protocol || uri.protocol === 'file:')) {
      ...
      if (uri && this.errorReportingService) {
        // we've got a URI, but it isn't a file or relative path,
        // so we don't support it.  Send telemetry so we know about it.
