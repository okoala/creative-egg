...
import { createHttpClientError, IErrorReportingService } from '@glimpse/glimpse-common';
...
export class ClientRequestInspector {
    ...
    public before(masterData: IRequestData): void {
        if (masterData.context) {
            const frame: IStackFrame = this.stackHelper.tryGetFirstUserCodeFrame(this.stackHelper.captureStack(this.before, 20));
            ...
            // tslint:disable-next-line:no-any
            const headers = _.assign({}, (<any>masterData.request)._headers);
            ...
            const payload: IDataHttpRequestPayload = {
                ...
                body: HttpHelper.createMessageBodyProperty(masterData.request, masterData.requestBodyChunks, masterData.requestBodyLength, this.agent.providers.configSettings, masterData.multiPartFormSummarizer ? masterData.multiPartFormSummarizer.getParts() : []),
                timing: {
                    startTime: 0
                },
            ...
            const message = this.messageConverter.createMessageEnvelope<IDataHttpRequestPayload>(['data-http-request', 'call-stack'], undefined, masterData.context);
    ...
    public after(masterData: IRequestData): void {
        if (masterData.context && masterData.responseMessage) {
            const payload: IDataHttpResponsePayload = {
                ...
                body: HttpHelper.createMessageBodyProperty(masterData.response, masterData.responseBodyChunks, masterData.responseBodyLength, this.agent.providers.configSettings),
                timing: {
            ...
            const transmittableMessage = this.agent.providers.messageConverter.transformMessageForTransit(masterData.responseMessage, payload);
    ...
    private onRequestDataSent(event: IInstrumentationEvent): void {
        ...
        if (masterData.context) {
            ...
            if (masterData.isMultiPartFormData === undefined) {
                masterData.multiPartFormSummarizer = createMultiPartFormSummarizer(masterData.request.getHeader('content-type'));
    ...
    private onResponseReceived(event: IInstrumentationEvent): void {
        ...
        if (masterData.context) {
            ...
            masterData.responseMessage = this.agent.providers.messageConverter.createMessageEnvelope<IDataHttpResponsePayload>(['data-http-response'], undefined, masterData.context);
