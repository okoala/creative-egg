...
export class MongoDBOptions {
  ...
  /**
   * Return the MongoDBOptions instance from the given object, or return undefined if not available.
   */
  public static get(options: object): MongoDBOptions {
    let gopts: MongoDBOptions;
    if (options) {
      gopts = options[MongoDBOptions.CUSTOM_KEY];
    }
    return gopts;
  }

  /**
   * Remove the MongoDBOptions instance from the given object
   */
  public static clear(options: object) {
    if (options && options[MongoDBOptions.CUSTOM_KEY]) {
      delete options[MongoDBOptions.CUSTOM_KEY];
    }
  }

  /**
   *  Ensure the MongoDBOptions instance exists on the given object, and return it.
   */
  public static ensure(options: object): MongoDBOptions {
    let gopts: MongoDBOptions = options[MongoDBOptions.CUSTOM_KEY];
    if (!gopts) {
      gopts = new MongoDBOptions();
      options[MongoDBOptions.CUSTOM_KEY] = gopts;
    }
    return gopts;
  }
...
export class MongoDBInspector {
  ...
  private insertInspector(event: IInstrumentationEvent) {
    ...
    MongoDBOptions.ensure(event.data.originalArgs[1]).operationName = 'insert';
  }
  ...
  private insertMethodsInspector(event: IInstrumentationEvent) {
    ...
    const opts: MongoDBOptions = MongoDBOptions.get(options);
    if (opts) {
  ...
  private readStartInspector(event: IInstrumentationEvent) {
    const originalThis = event.data.originalThis;
    const cursorCustomOpts = MongoDBOptions.ensure(originalThis.s.cmd);
    if (!cursorCustomOpts.getMessageSent('data-mongodb-read-start')) {
      ...
      // as this will be present if close() is called before next()/each()/forEach()/...
  ...
  private readRecordInspector(event: IInstrumentationEvent) {
    ...
    if (event.data.result) {
      ...
      // - for each(), next() will be invoked by the underlying implementation,
      // so we don't want to double-count records.
