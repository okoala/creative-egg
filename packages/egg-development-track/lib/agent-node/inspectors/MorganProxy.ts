...
export class MorganProxy extends ProxyBase {
  ...
  public init(agent: IAgent, module) {
    const newModule = function newModule() {
      const logger = module.apply(this, arguments) as IMiddlewareFunction;
    ...
    // We need to keep the property on the new module for compatibility purposes so the warning, while annoying,
    // must be ignored.
