...
export class BodyParserProxy extends ProxyBase {
  ...
  private static wrapParserFactory(parserFactory, name: string, displayName: string): IMiddlewareFunction {
    return function newParserFactory() {
      const parser = parserFactory.apply(this, arguments) as IMiddlewareFunction;
  ...
  public init(agent: IAgent, module) {
    ...
    // We need to keep the function in the new module for compatibility purposes
    // so the warning, while annoying, must be ignored.

    /* tslint:disable no-any */
    const newModule: any = BodyParserProxy.wrapParserFactory(module, 'bodyParser', 'Body Parser');
    /* tslint:enable no-any */

    // NOTE: The original module explicitly
    // defines these exports as read-only properties so we must create a new module (function).
    ...
    newModule.urlencoded = BodyParserProxy.wrapParserFactory(
      module.urlencoded, 'urlencoded', 'URL-Encoded Body Parser',
    );
